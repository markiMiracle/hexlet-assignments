...
public class ProductsController {
    ...
    @PutMapping("/{id}")
    public ProductDTO update(@PathVariable("id") String id, @RequestBody ProductUpdateDTO updateData) {
        var product = productRepository.findById(Long.parseLong(id))
                .orElseThrow(() -> new ResourceNotFoundException("Product with id " + id + " not found"));
        updateDTO(updateData, product);
        productRepository.save(product);
        return toDTO(product);
    }
    private Product updateDTO(ProductUpdateDTO updateData, Product product) {
